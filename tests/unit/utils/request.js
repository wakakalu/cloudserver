...
function createProxyServer(targetHost, hostname, port, callback) {
    const target =  new URL(targetHost);
    const proxy = http.createServer((uReq, uRes) => {
...
[
    ...
    describe(`test against ${protocol} server`, () => {
        ...
        describe('request', () => {
            it('should set proxy agent if proxy envs are set', done => {
